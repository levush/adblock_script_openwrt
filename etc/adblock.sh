...
add_config()
{
    ...
    #Update DHCP config
    uci add_list dhcp.@dnsmasq[0].addnhosts=/tmp/adblock/block.hosts > /dev/null 2>&1 && uci commit
...
update_blocklist()
{
    ...
    if [ -s "/tmp/adblock/black.list" ]
    then
        echo 'Adding blacklist...'
        awk -v r="$ENDPOINT_IP4" '/^[^#]/ { print r,$1 }' /tmp/adblock/black.list >> /tmp/block.build.list
    fi
    ...
    if [ -s "/tmp/adblock/white.list" ]
    then
        #Filter the blacklist, supressing whitelist matches
        #  This is relatively slow =-(
        echo 'Filtering white list...'
        egrep -v "^[[:space:]]*$" /tmp/adblock/white.list | awk '/^[^#]/ {sub(/\r$/,"");print $1}' | grep -vf - /tmp/block.build.before > /tmp/adblock/block.hosts
    else
        cat /tmp/block.build.before > /tmp/adblock/block.hosts
    fi
    ...
    then
        ...
        sed -i -re "s/^(${safe_pattern}) (.*)$/\1 \2\n${safe_addition} \2/g" /tmp/adblock/block.hosts
    fi
}
...
remove_config()
{
    ...
    # Remove addnhosts
    uci del_list dhcp.@dnsmasq[0].addnhosts=/tmp/adblock/block.hosts > /dev/null 2>&1 && uci commit
